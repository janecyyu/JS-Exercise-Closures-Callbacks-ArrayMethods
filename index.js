...
console.log(processProduct(10, 20, total => total + " is a big number!"));
...
processContains("foo", `['foo', 'bar']`, boolean => {
  boo(boolean);
});

processContains("lady gaga", `['foo', 'bar']`, boolean => {
  boo(boolean);
});
...
function processDuplicateFree(list, callback) {
  ...
  return callback(list.filter((item, i) => list.indexOf(item) === i));
}

const testList = [1, 2, 3, 4, 4, 5, 6, 7, 7, 7];

processDuplicateFree(testList, list => console.log(list));
...
function firstNamesAllCaps(arr) {
  /* CODE HERE */
  let newArr = arr.map(runner => runner.first_name.toUpperCase());
  return newArr;
}
 ...
 */
function getRunnersByTShirtSize(arr, tShirtSize) {
  /* CODE HERE */
  let newArr = arr.filter(runner => runner.shirt_size === tShirtSize);
  return newArr;
}
console.log(getRunnersByTShirtSize(runners, "XS"));
/**
...
function tallyUpDonations(arr) {
  /* CODE HERE */
  let sum = arr.reduce((total, runner) => {
